...
dos_param <- 
  data.frame(
    ncoe = ncoe,
    gain = c(0.668706, 1.31702, 1.03988, 0.872588, 0.119882,
             0.0652941),
    bias = c(-1.52, -2.84, -1.17, -1.51, -0.37, -0.15),
    edist = landsat::ESdist("1986-01-06"),
    # https://earthexplorer.usgs.gov/
    sunelev = 52.67848915, 
    # https://landsat.usgs.gov/esun
    Esun = c(1958, 1827, 1551, 1036, 214.9, 80.65)
  ) %T>% 
  print()
```
...
dos <- 
  landsat::DOS(
    ...
    SHV = 55, 
    SHV.band = 1,
    Grescale = dos_param$gain,
    Brescale = dos_param$bias,
    edist = dos_param$edist, 
    sunelev = dos_param$sunelev)$DNfinal.mean %T>% 
  print()
```


```{r tmp}
dos_param$coef <- dos["coef-4"]
```

```{r tmp}
landsat_dos <- 
  parallel::mclapply(1:6, function (i) {
    res <-
      landsat[[i]] %>% 
